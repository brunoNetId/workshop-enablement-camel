...
[NOTE] 
--
* The Java file has a header that includes a Camel K flag to indicate the dependency required (`dependency=camel-ahc-ws`).

* The class needs to extend `RouteBuilder` as _Camel K_ only accepts route builder definitions. In our case we define an empty `configure` method.
