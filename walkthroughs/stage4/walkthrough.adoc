...
_Discord_ is a well known chat platform, very popular among gamers, but more and more embraced by enterprises for internal communications and to keep in touch and interact with their clients.

The difficulty of this new stage is that we don't have in our catalog _Kamelets_ for _Discord_. We need to create our own connectors.
...
- Craete a _Kafka_ to _Discord_ flow using the new _Kamelet_
- Create a _Discord_ to _Kafka_ flow using _Camel K_ (DSL)

{empty} +

and optionally:

- Create a _Telegram_ to _Kafka_ flow
- Create a _Kafka_ to _Teletram_ flow
