...
* Trigger the `wait-until-done` Camel route. +
This action maintains an _Exchange_ always alive (in-flight) to prevent the cronjob from terminating _Camel_ too early. 
--

* A method `waitUntilJobDone` that waits for the latch to be released. +
This method is invoked from the `wait-until-done` _Camel_ route. +
{empty} +

* A method `jobDone` to announce the run has completed. +
The aggregator logic invokes this method to signal completion. +
{empty} +

* A method `expiredTimeWindow` to return the status of the variable. +
...
Now you can see how the Controller and Aggregation are involved in the process.
...
{empty} +
+
Make sure you save all the changes done.
+
{empty} +
