...
. [[step8]]Replace your Log sink Kamelet by the Slack one.
+
...
. Configure the `channel` in your JSLT transformation.
+
...
```
When Slack receives the JSON data, it will know it targets your room, based on the `channel` value.
+
{empty} +

. Include your Slack token in your configuration file.
+
Copy from below the `slack.token` parameter, paste it into your properties file, and set its value to your Slack access token value.
+
----
# Allows HTTP streaming from Gitter
camel.main.streamCachingEnabled=false
...
```
+
* If you're sharing an App with the group, use the App token your admin provides you with.
* If you created your own App, use your App's `Bot User OAuth Token`
+
{empty} +

. Run from your terminal Camel JBang as follows:
+
```bash
camel run g2s.yaml g2s.jslt \
--local-kamelet-dir=$PWD/../kamelets \
...
. Once again, from Gitter send one last message. If all goes well you should see the message listed in your Slack chat window
+
