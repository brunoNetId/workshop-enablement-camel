...
+
Ensure you select your target OpenShift working project, for example by running:
+
```bash
...
```
+
Them create a Secret containing your configuration. Run the following `oc` command:
+
```bash
...
Your pod logs should show the arrival of a Gitter event similar to the following JSON payload:
+
```json
...
```
+
The Camel K operator needs again to download the JSLT dependencies, but after some time it will replace the pod with a new instance containing the data transformation.
+
{empty} +
